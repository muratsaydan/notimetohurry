...
**Kurulu n8n workflow:** [Slow Living Radar](https://n8n.saydan.net/workflow/NpuUHH9TzEKZS2hu) — **iki RSS** (Slow Living Collective + Google News; Zen Habits listeden çıkarıldı), **yalnızca son 7 günde** `pubDate`/`isoDate`’i olan öğeler, linke göre tekilleştirme, **Google Vertex AI** üzerinden **Gemini** (`Basic LLM Chain` + `Google Vertex Chat Model`, hizmet hesabı `googleApi`) ile her başlığın altında **Türkçe özet** (tek JSON nesnesi). Yerelde şablonu üretirken `secrets.env` içine **GCP_VERTEX_PROJECT_ID** (veya `GCP_PROJECT_ID`) ve isteğe bağlı **GCP_VERTEX_MODEL** (varsayılan `gemini-2.0-flash`) yazın; kimlik bilgisindeki **Vertex bölgesi** (`region`) modelin o projede kullanılabildiği yerle eşleşmelidir. **Gmail** ile `murat@saydan.net`. `Apply Turkish summaries` Chain çıktısındaki `text` alanını da okur; ayrıca (yedek) doğrudan Gemini API / OpenAI biçimlerine düşebilir. HTML’de UTF-8 BOM + `Content-Type` meta vardır. **Zamanlama:** `Her Pazartesi 08:00` düğümü (haftalık); otomatik posta için workflow’u n8n’de **Active** yapın. İstediğiniz zaman **Test workflow** ile manuel de çalıştırabilirsiniz. Şablonu yerelde güncellemek: `node deploy/build-slow-living-radar-workflow.mjs` (`N8N_BASE_URL` + `N8N_API_KEY` varsa aynı komut sunucuya da PUT eder).
